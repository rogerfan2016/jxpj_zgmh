  ...
  <head>
    ...
    <script type="text/javascript">
      ...
      function refreshList() {
        var form = $("#search");
        form.attr("action", "<%=request.getContextPath()%>/evaluation/class_checkClassInstructionsList.html");
        form.submit();
      }
             ...
             <tr>
              ...
              <td>
                <select name="query.zt" style="width: 100px">
                  <option value="" selected>全部</option>
                  <option value="1" <c:if test="${query.zt eq '1' }">selected</c:if>>待审核</option>
                  <option value="2" <c:if test="${query.zt eq '2' }">selected</c:if>>已审核</option>
                </select>
              </td>
            </tr>
          </tbody>
            ...
            <tbody id="list_body">
              ...
              <tr name="tr">
                ...
                <c:if test="${info.zt eq '1' }">
                	<a style="color:#074695" onclick="showWindow('任课说明书详情', '<%=request.getContextPath()%>/evaluation/class_checkClassInstructionsDetailList.html?id=${info.id }', '700','550');" href="#">审核</a>
                </c:if>
                <c:if test="${info.zt eq '2' or info.zt eq '3' or info.zt eq '4' }">
                	<a style="color:#074695" onclick="showWindow('任课说明书详情', '<%=request.getContextPath()%>/evaluation/class_checkClassInstructionsDetailList.html?id=${info.id }', '700','550');" href="#">查看</a>
                </c:if>
                </td>
                <td>
                	<c:if test="${info.zt eq '1' }">待审核</c:if>
                	<c:if test="${info.zt eq '2' }"><font color="green">审核通过</font></c:if>
