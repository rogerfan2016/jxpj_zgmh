...
<html>
    ...
    <script type="text/javascript">
      $(function() {
	       ...
	       $("#btn_dc").click(function() {
	         	window.open("<%=request.getContextPath()%>/feedback/handle_unitExportFeedBack.html?" + $("#search").serialize());
	       });
      });
      
      //反馈信息处理
      function unitHandleFeedback(xxid){
      		var url=_path+'/feedback/handle_unitHandleFeedbackView.html?xxid='+xxid;
			showWindow('信息处理',url,600,400);
      }
      //反馈操作
      function unitHandleAddFeedback(xxid){
      		var url=_path+'/feedback/handle_unitHandleAddFeedbackView.html?xxid='+xxid;
			showWindow('反馈操作',url,550,400);
      }
  ...
  <body>
    ...
    <form action="<%=request.getContextPath()%>/feedback/handle_unitHandleFeedBackList.html" name="search" id="search" method="post">
          ...
          <table width="100%" class="dateline tablenowrap">
            ...
            <tbody id="list_body">
              ...
              <tr name="tr" align="center"">
              	<td>
              		<c:if test="${info.clhj eq '0' && info.xxnrlx != '0'}"><a id="handle" class="btn_xg" onclick="unitHandleFeedback('${info.xxid}');"><font color="blue">处理</font></a></c:if>
              		<c:if test="${info.clhj eq '2' && info.xxnrlx != '0'}"><a id="feedback" class="btn_xg" onclick="unitHandleAddFeedback('${info.xxid}');"><font color="green">反馈</font></a></c:if>
              		<c:if test="${ (info.clhj != '2' && info.clhj != '0') || info.xxnrlx eq '0' }"><a id="detail" class="btn_xg" onclick="feedBackDetail('${info.xxid}');"><font color="blue">查看</font></a></c:if>
              	</td>
              	<td>${info.xn}</td>
              	<td>
