...
<html>
  ...
  <body>
    <form action="<%=request.getContextPath()%>/feedback/info_feedBackList.html" name="search" id="search" method="post">
        ...
        <table width="100%" border="0">
          <tbody>
            ...
            <th>学年</th>
              ...
              <th>信息类型</th>
              <td>
                <select name="query.xxlx" style="width:100px;">
                  <option value=""></option>
                  <option value="0" <c:if test="${query.xxlx eq '0'}">selected</c:if>>普通</option>
                  <option value="1" <c:if test="${query.xxlx eq '1'}">selected</c:if>>紧急</option>
                </select>
              </td>
              </tr>
            <tr>
             <th>内容类型</th>
              ...
              <th>处理环节</th>
              <td>
                <select name="query.clhj" style="width:100px;">
                  <option value=""></option>
                  <option value="0" <c:if test="${query.clhj eq '0'}">selected</c:if>>单位管理员筛选</option>
                  <option value="1" <c:if test="${query.clhj eq '1'}">selected</c:if>>责任人/单位处理</option>
                  <option value="2" <c:if test="${query.clhj eq '2'}">selected</c:if>>单位管理员反馈</option>
                  <option value="3" <c:if test="${query.clhj eq '3'}">selected</c:if>>信息员评价</option>
                  <option value="4" <c:if test="${query.clhj eq '4'}">selected</c:if>>完成</option>
                </select>
              </td>
              <th>状态</th>
              <td>
                <select name="query.zt" style="width:80px;">
                  <option value=""></option>
                  <option value="0" <c:if test="${query.zt eq '0'}">selected</c:if>>已提交</option>
                  <option value="1" <c:if test="${query.zt eq '1'}">selected</c:if>>处理中</option>
                  <option value="2" <c:if test="${query.zt eq '2'}">selected</c:if>>已反馈</option>
                  <option value="3" <c:if test="${query.zt eq '3'}">selected</c:if>>已评价</option>
                  <option value="4" <c:if test="${query.zt eq '4'}">selected</c:if>>已退回</option>
                </select>
              </td>
            </tr>
          </tbody>
          ...
          <table width="100%" class="dateline tablenowrap">
            <thead id="list_head">
              <tr>
              	...
              	<td width="10%" style="text-align:center;">学年</td>
              	<td width="10%" style="text-align:center;">学期</td>
            ...
            <tbody id="list_body">
              ...
              <tr name="tr" align="center"">
              	<td><a id="detail" class="btn_xg" onclick="feedBackDetail('${info.xxid}');"><font color="blue">详情</font></a></td>
              	<td>${info.xn}</td>
              	<td>第${info.xq}学期</td>
              	<td>
