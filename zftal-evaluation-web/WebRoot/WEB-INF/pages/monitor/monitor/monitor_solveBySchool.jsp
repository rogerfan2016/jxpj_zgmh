...
<html>
  <head>
    ...
    <script type="text/javascript">
      ...
      function modifyDetail(id){
      	showWindow("审核巡视记录", "<%=request.getContextPath()%>/monitor/monitor_auditingBySchool.html?detail.id=" + id, "400","400");
      }
      
      function refreshList() {
        var form = $("#search");
        form.attr("action", "<%=request.getContextPath()%>/monitor/monitor_solveBySchool.html");
        form.submit();
      }
    </script>
  </head>
  ...
  <div class="toolbox">
    ...
    <form action="<%=request.getContextPath()%>/monitor/monitor_solveBySchool.html" name="search" id="search" method="post">
      <div class="searchtab">
            ...
            <tr>
              ...
              <td  colspan="5">
                <select name="query.zt" style="width: 100px">
                  <option value="" selected>全部</option>
                  <option value="2" <c:if test="${query.zt eq '2' }">selected</c:if>>未审核</option>
        ...
        <div class="con_overlfow">
          <table width="100%" class="dateline tablenowrap">
            ...
            <tbody id="list_body">
              ...
              <tr name="tr">
              	<td>
                	<c:if test="${info.zt eq '2' }"><a style="color:#074695" href="#" onclick="modifyDetail(${info.id });">审核处理</a></c:if>
                	<c:if test="${info.zt eq '3' }">审核通过</c:if>
                	<c:if test="${info.zt eq '4' }">审核不通过</c:if>
                </td>
                <td>${info.xxclyj }</td>
                <td>${info.yxclyj }</td>
