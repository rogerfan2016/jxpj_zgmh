...
WjModel_TiMu.prototype.getTmPropertyHiddenHTML = function(e_name,e_value){
	...
	jQuery(hiddenElement).attr("type","hidden");
	jQuery(hiddenElement).attr("name",e_name);
	jQuery(hiddenElement).attr("value",e_value?e_value:"");
	//hiddenElement.NAME=e_name;
	//hiddenElement.VALUE=(e_name);
	...
	if(WjModel_Parameter.wjlx==WjModel_WJLX.CPL&&tm.stlx!=WjModel_STLX.STDL){
		if(tm.stzf!=""){
			if(xxs.length>0){
				...
				if(xxfz_mark){
					if(tm.stzf!=xxfz_total&&tm.stlx==WjModel_STLX.DXS){
						msg+="试题分值与其选项分值不一致！";
					}
				}else{
					msg+="未设置选项分值！";
